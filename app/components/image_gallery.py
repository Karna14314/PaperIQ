...
def render_image_gallery(
    ...
    Args:
        ...
        show_metadata: Whether to show image metadata
    """
    if not images:
            ...
            if i + j < len(images):
                ...
                with col:
                    ...
                    if img_path.exists():
                        st.image(
                            str(img_path),
                            caption=f"Page {img.page_number}",
                            use_container_width=True
                        )
                        
                        if show_metadata:
                            st.caption(f"{img.width}×{img.height} • {img.format}")
                    else:
                        st.warning("Image not found")


def render_image_detail(image) -> None:
    """
