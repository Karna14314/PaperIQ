...
def render_validation_report(report, paper) -> None:
    """
    ...
    with col2:
        ...
        <div style="text-align: center; padding: 1rem; background-color: #fef3c7; border-radius: 8px;">
            <div style="font-size: 1.5rem; font-weight: bold; color: #92400e;">⚠️ {report.warning_count}</div>
...
def render_quality_score(score: float, level: str) -> None:
    """
    ...
    ">
        <div style="font-size: 3rem; font-weight: bold; color: {text_color};">
            {score:.0%}
        </div>
        <div style="font-size: 1.2rem; color: {text_color}; text-transform: uppercase; letter-spacing: 0.1em;">
            {level} Quality
        </div>
    </div>
    """, unsafe_allow_html=True)
...
def render_validation_item(item) -> None:
    """
    Render a single validation item.
    
    Args:
        item: ValidationItem object
    """
    styles = {
        "pass": {
            "bg": "#d1fae5",
    ...
    ">
        <div style="color: {style['text']}; font-weight: 600;">
            {style['emoji']} {item.name}
        </div>
        <div style="color: {style['text']}; opacity: 0.8; font-size: 0.9rem;">
            {item.message}
        </div>
    </div>
    """, unsafe_allow_html=True)
